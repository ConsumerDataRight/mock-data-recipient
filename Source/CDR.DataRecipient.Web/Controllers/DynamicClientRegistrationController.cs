...
namespace CDR.DataRecipient.Web.Controllers
{
    ...
    public class DynamicClientRegistrationController : Controller
    {
        ...
        {
            this._config = config;
            this._regRepository = regRepository;
            this._dhRepository = dhRepository;
            this._ssaService = ssaService;
            this._dcrService = dcrService;
            this._regInfosecService = regInfosecService;
        ...
        public async Task<IActionResult> Index(string clientId = null, string dataHolderBrandId = null)
        {
            ...
            {
                this.SetViewModelDefaults(model);
            }
            else
            {
                var client = await this._regRepository.GetRegistration(clientId, dataHolderBrandId);
                model.TransactionType = "Update";
                this.SetViewModel(model, client);
            }

            await this.PopulateFormDetail(model);
        ...
        public async Task<IActionResult> Index(DynamicClientRegistrationModel model)
        {
            try
            {
                ...
                {
                    await this.UpdateRegistration(model);
                }
        ...
        public async Task<IActionResult> Delete(string clientId, string dataHolderBrandId)
        {
            ...
            try
            {
                var deleteResponse = await this.DeleteRegistration(clientId, dataHolderBrandId);
        ...
        public async Task<IActionResult> Get(string clientId, string dataHolderBrandId)
        {
            ...
            try
            {
                var reg = await this.GetRegistration(clientId, dataHolderBrandId);
        ...
        public async Task<IActionResult> UpdateDCRRegistration(string clientId, string dataHolderBrandId)
        {
            ...
            try
            {
                regResp = await this.UpdateCurrentRegistration(clientId, dataHolderBrandId);
            }
            catch (Exception ex)
            {
                var msg = $"Unable to view the DCR details for ClientId: {clientId}, DataHolderBrandId:{dataHolderBrandId} - {ex.Message}";
        ...
        public async Task<IActionResult> GetOidcDocument(string dataHolderBrandId)
        {
            ...
            try
            {
                ...
                dataHolder = await this._dhRepository.GetDataHolderBrand(dataHolderBrandId);
                ...
                else
                {
                    ...
                    var oidcDiscovery = await this._dataHolderDiscoveryCache.GetOidcDiscoveryByBrandId(dataHolderBrandId);
        ...
        private async Task Register(DynamicClientRegistrationModel model)
        {
            ...
            var ssa = await this.GetSSA(sp, model);

            // Construct the DCR request.
            var dataHolderDiscovery = await this._dataHolderDiscoveryCache.GetOidcDiscoveryByBrandId(model.DataHolderBrandId);
            ...
            var dcrResponse = await this._dcrService.Register(
            ...
            if (dcrResponse.IsSuccessful)
            {
                ...
                await this._regRepository.PersistRegistration(dcrResponse.Data);
            }
        }
        ...
        private async Task<DynamicClientRegistrationModel> GetRegistration(string clientId, string dataHolderBrandId)
        {
            ...
            var dataHolderDiscovery = await this._dataHolderDiscoveryCache.GetOidcDiscoveryByBrandId(client.DataHolderBrandId);
            ...
            // Request DCR to the Data Holder.
            var dcrResponse = await this._dcrService.GetRegistration(
                dataHolderDiscovery.RegistrationEndpoint,
                sp.ClientCertificate.X509Certificate,
        ...
        private async Task<ResponseModel> UpdateCurrentRegistration(string clientId, string dataHolderBrandId)
        {
            ...
            var softwareProductConfig = this._config.GetSoftwareProductConfig();
            var registerConfig = this._config.GetRegisterConfig();
            var tokenEndpoint = await this._cacheManager.GetRegisterTokenEndpoint(registerConfig.OidcDiscoveryUri);

            // Get an access token from the Register.
            var registerTokenResponse = await this._regInfosecService.GetAccessToken(
                tokenEndpoint,
            ...
            var dataHolderDiscovery = await this._dataHolderDiscoveryCache.GetOidcDiscoveryByBrandId(dataHolderBrandId);
            ...
            var dhTokenResponse = await this._dhInfosecService.GetAccessToken(accessToken);
            ...
            var dcrResponse = await this._dcrService.UpdateRegistration(
                dataHolderDiscovery.RegistrationEndpoint,
                softwareProductConfig.ClientCertificate.X509Certificate,
            ...
            if (dcrResponse.IsSuccessful)
            {
                ...
                await this._regRepository.UpdateRegistration(dcrResponse.Data);
            }
        ...
        private async Task<DynamicClientRegistrationModel> DeleteRegistration(string clientId, string dataHolderBrandId)
        {
            ...
            var dcrResponse = await this._dcrService.DeleteRegistration(
                dataHolderDiscovery.RegistrationEndpoint,
                sp.ClientCertificate.X509Certificate,
                tokenResponse.Data.AccessToken,
                clientId);
            ...
            await this._regRepository.DeleteRegistration(clientId, dataHolderBrandId);
        ...
        private async Task<string> GetSSA(SoftwareProduct sp, DynamicClientRegistrationModel model)
        {
            ...
            // Get an access token from the Register.
            var tokenResponse = await this._regInfosecService.GetAccessToken(
                tokenEndpoint,
        ...
        {
            var allowDynamicClientRegistration = await this._featureManager.IsEnabledAsync(nameof(Feature.AllowDynamicClientRegistration));

            // Return any from the Registration table repository.
            var registrations = await this._regRepository.GetRegistrations();
            model.DataHolderBrands = (await this._dhRepository.GetDataHolderBrands())
            ...
            if (!allowDynamicClientRegistration)
            {
                ...
                var dcrMessages = await this._regRepository.GetDcrMessageRegistrations();
