...
namespace CDR.DataRecipient.SDK.Services.Register
{
    public class SsaService : BaseService, ISsaService
    {
            ...
            string softwareProductId)
        {
            ...
            var ssaEndpoint = $"{mtlsBaseUri}/cdr-register/v1/banking/data-recipients/brands/{brandId}/software-products/{softwareProductId}/ssa";

            // Setup the http client.
            var client = GetHttpClient(clientCertificate, accessToken, version);

            _logger.LogDebug($"Requesting SSA from Register: {ssaEndpoint}.  Client Certificate: {clientCertificate.Thumbprint}");

            // Make the request to the get data holder brands endpoint.
            var response = await client.GetAsync(ssaEndpoint);
            var body = await response.Content.ReadAsStringAsync();

            _logger.LogDebug($"Get SSA Response: {response.StatusCode}.  Body: {body}");
