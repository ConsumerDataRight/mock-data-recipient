    ...
    public static class RequestExtensions
    {
        public static (List<Claim>, string) CreateClaimsForDCRRequest(this DcrRequest dcrRequest)
        {
            ...
            var claims = new List<Claim>
            {
                ...
                new("grant_types", TokenTypes.REFRESH_TOKEN)
            };

            // response_types updated below "code" is returned and added below
            // A response type is mandatory
            if (!dcrRequest.ResponseTypesSupported.Contains("code"))
            {
                // Return the error                 
            ...
            var responseTypesList = dcrRequest.ResponseTypesSupported.Where(x => x.ToLower().Equals("code")).ToList();
            claims.Add(new Claim("response_types", JsonConvert.SerializeObject(responseTypesList), JsonClaimValueTypes.JsonArray));


            if (dcrRequest.AuthorizationSigningResponseAlgValuesSupported.Length == 0)
            {
            ...
            if (dcrRequest.AuthorizationEncryptionResponseAlgValuesSupported != null && dcrRequest.AuthorizationEncryptionResponseAlgValuesSupported.Length != 0)
            {
